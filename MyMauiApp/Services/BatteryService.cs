...
public class BatteryService : IBatteryService
{
    ...
    private const string DefaultDeviceName = "Amped 24v";
    ...
    public async Task<bool> ConnectAsync()
    {
        ...
        var targetName = _selectedDevice?.Name ?? DefaultDeviceName;
        var targetId = _selectedDevice?.Id;

        SetStatus($"Scanning for {_selectedDevice?.DisplayName ?? DefaultDeviceName}...");
        ...
        {
            // Match by ID if we have a selected device, otherwise by name
            if (targetId != null && e.Device.Id == targetId)
            {
                batteryDevice = e.Device;
                deviceFoundTcs.TrySetResult(e.Device);
            }
            else if (targetId == null && e.Device.Name == targetName)
            {
                batteryDevice = e.Device;
                deviceFoundTcs.TrySetResult(e.Device);
            }
        ...
        if (batteryDevice == null)
        {
            SetStatus($"{_selectedDevice?.DisplayName ?? DefaultDeviceName} not found");
            return false;
        }
